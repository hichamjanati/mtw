...
@cython.wraparound(False)
@cython.cdivision(True)
cdef floating[:] cython_solver(int n_samples, int n_features,
    ...
    for i in range(maxiter):
        ...
        for j in range(n_features):
            ...
            old_j = theta[j]
            if a > 0. or positive:
                theta[j] = proximal(tmp, marginals[j] * Ls[j], a, b * sigma[0]) / Ls[j]
            else:
                theta[j] = fsign(tmp) * fmax(fabs(tmp) - b * sigma[0], 0.) / Ls[j]
...
def cython_wrapper(int n_samples, int n_features, floating[::1, :] X,
                   ...
                   floating a, floating b, floating sigma0=0.,
                   floating tol=1e-4, int maxiter=10000,
                   bint positive=0):
    cdef:
        floating[:] sol
    with nogil:
        sol = cython_solver(n_samples, n_features, X, y, Ls,
                            marginals, theta, R, sigma, a, b,
                            sigma0, tol, maxiter, positive)
    return np.asarray(sol), np.asarray(R), np.asarray(sigma)
